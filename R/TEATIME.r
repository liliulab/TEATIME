...
#' @param collect.data An optional Dataframe, we prepare it for normal case.
...
#' @param vaf_set provide vafs for main cluster
...
#' @examples determine_mupick(NA, 0.2, 0.1, 0.3, 2.5, 3.0)
...
#' @param input.file: input for magos if steps has 0; OR, magos object,i.e, run, OR
#' manually, it should be dataframe with columns: vaf.1, depth.1,colors
#' colors refers to the cluster corresponding to each mutation
#' @param beta Numeric, a fixed survival rate. Default is 0.9
#' @param depth Numeric, a fixed value. Only enter if you know exactly what the value is.
#' @param p_thre: p value cutoff used in most function. Default is 0.01
#' @param magos_object Check if input is from magos ouput
...
#' @param steps: A vector of integers indicating which functions to execute. 0 means run MAGOS
#' @param write_final: if we want to write final result in txt file
#' @param debug_mode: If we want to show error
#' @return NULL
#' @export
