...
run_estimates <- function(ctx, p_thre = 0.01) {
  ...
  mc_cores <- getOption("teatime.mc.cores",
  ...
  jobs <- expand.grid(estimator = estimator_names, try_idx = 1:3,
  ...
  n_jobs <- nrow(jobs)
  if (.Platform$OS.type == "unix" && mc_cores > 1L && n_jobs > 1L) {
    job_out <- parallel::mclapply(seq_len(n_jobs), run_job,
                                  mc.cores = min(n_jobs, mc_cores),
                                  mc.set.seed = TRUE)
  } else {
    job_out <- lapply(seq_len(n_jobs), run_job)
  }
